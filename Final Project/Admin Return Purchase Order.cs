...
{
    public partial class Admin_Return_Purchase_Order : Form
    {
        public Admin_Return_Purchase_Order()
        {
            InitializeComponent();
        }
        ...
        private void rid_auto_increment()
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    string sql1 = "SELECT MAX(returnOrderId) FROM ReturnOrders";
                    using (SqlCommand cmd = new SqlCommand(sql1, con))
                    {
                        ...
                        if (dr.Read())
                        {
                            ...
                            if (!string.IsNullOrEmpty(maxItemId) && maxItemId.StartsWith("RO"))
                            {
                                ...
                                if (int.TryParse(numericPart, out int maxID))
                                {
                                    ...
                                    this.txtroid.Text = "RO" + newID.ToString("D2"); // D2 ensures at least two digits
                                }
                                else
                                {
                                    // Handle the case where the numeric part is not valid
                                    this.txtroid.Text = "RO01";
                                }
                            }
                            else
                            {
        ...
        private void Admin_Return_Purchase_Order_Load(object sender, EventArgs e)
        {
            adminidcombox();
            productcombo();
            suppliercombo();
        ...
        private void insert_data_into_returnorderconsist()
        {
            //connection
            string cs = @"Data Source=HPNotebook; 
            Initial Catalog=DSE_FinalProject; 
            ...
            //Command
            string sql = "INSERT INTO ReturnOrderConsists (returnOrderId,hardwareProductId,quantity,subTotal) VALUES(@poid,@hid,@qty,@subtotal)";
            SqlCommand com = new SqlCommand(sql, con);
            com.Parameters.AddWithValue("@poid", this.txtroid.Text);
        ...
        private void data_load_to_grid()
        {
            ...
            string sql = "SELECT * FROM ReturnOrderConsists WHERE returnOrderId=@pid";
            SqlCommand com = new SqlCommand(sql, con);
            com.Parameters.AddWithValue("@pid", this.txtroid.Text);
        ...
        private void btnmark_Click(object sender, EventArgs e)
        {
            ...
            {
                calculate_the_total();
                insert_data_into_returnorderconsist();
                Insert_inventory_quntity();
                data_load_to_grid();
                textbox_clear();
                    ...
                    using (SqlCommand com1 = new SqlCommand(sql1, con))
                    {
                        ...
                        using (SqlDataReader dr = com1.ExecuteReader())
                        {
                            if (dr.Read())
                            {
                                ...
                                string sql = "INSERT INTO ReturnOrders(returnOrderId, totalAmount, dateAndTime, adminId, supId) " +
                                             "VALUES(@rid, @tamount, @datetime, @aid, @sid)";
        ...
        private void Insert_inventory_quntity()
        {
            ...
            com.Parameters.AddWithValue("@pq", this.lableremaining.Text);
